<template>
    ...
    <div class="applyForm">
        ...
        <el-steps class="steps" :active="steps" align-center>
            <el-step @click.native="nextStep(0)" title="基本信息" description=""></el-step>
            <el-step @click.native="nextStep(1)" title="详细信息" description=""></el-step>
            <el-step @click.native="nextStep(2)" :title="business" description="选填信息可以跳过！"></el-step>
            <el-step @click.native="nextStep(3)" title="注册成功" description=""></el-step>
        </el-steps>
            ...
            <ul>
                ...
                <!--<li>
                    ...
                    <el-radio-group class="ascription" v-model="checkList">
                        ...
                        <el-radio :label="3">采购商和供应商</el-radio>
                    </el-radio-group>
                </li>-->
                <!--企业类型-->
                <li>
                ...
                <li>
                    <span>*公司电话：</span>
                    <el-input @blur="blurs('companyPhone')" size="mini" placeholder="请输入联系人号码" v-model="companyPhone" type="text"></el-input>
                    <b v-if="tip_companyPhone">请输入公司电话</b>
                </li>
            </ul>
            <el-button @click="nextStep(1)" class="submit" type="primary">下一步</el-button>
        </div>
        <div v-if="steps==1" class="form">
            ...
            <el-button @click="nextStep(2)" class="submit" type="primary">下一步</el-button>
        </div>
        <!--供应商-->
        <div v-if="steps==2&&checkList==0" class="form">
            <!--<ul>
                &lt;!&ndash;法定经营范围&ndash;&gt;
                <li>
                    <span>法定经营范围：</span>
            ...
            <el-button @click="nextStep(3)" class="submit" type="primary">保存</el-button>
        </div>
        <!--采购商-->
        <div v-if="steps==2&&checkList==1" class="form">
            <ul>
                <!--主营行业-->
                <li>
                    <span>主营行业：</span>
        ...
        <div v-if="steps==3" class="form">
            <div class="success">
                <i class="iconfont icon-right1"></i>
                <span>注册成功</span>
                <div class="dd">您已成功入驻ECCS，系统将在{{time}}s之后跳转至登录页!</div>
            </div>
    ...
    export default{
        ...
        data(){
            return{
                ...
                dialogImageUrl: '',/*图片预览*/
                /************************/
                storeEnter:'/open/api/acc/storeEnter',/*保存*/
                titles:'',
                business:'',
        ...
        methods:{
            ...
            nextStep(step){
                /*console.log(step)*/
                /*this.steps=step;*/
                let {license,times,licenseNumber,logo,companyEmployeeCount,yearIncome,RegisteredCapital,establishment,email,companyInfo,market,products,companyPhone,links,Contacts,companyName,TypesOfEnterprises,EnterpriseClassification,Country,companyAddress}=this;
                if(step==1){
                ...
                if(step==3&&this.time){
                    let {AccountPeriod,PaymentMethod,ProcurementScale,ProcurementCycle,managementModel,MainBusiness,AnnualExportVolume,AnnualImportVolume,AnnualTurnover,monthlyProduction,BrandName,NRDD,fileList}=this,
                    list=[];
                    fileList.map(item=>{
...
//                                this.steps=2;
        ...
        mounted(){
            ...
            let perfectStoreCnd=this.perfectStoreCnd;
            this.server(perfectStoreCnd,api.collection).then(res=>{
                if(res.code){
                    this.$message({
                        message: res.msg,
                        type: 'warning'
                    ...
                    this.CountryOptions=res.data.countryList;/*所在国家*/
                    this.EnterpriseClassificationOptions=res.data.storeClass;/*企业分类*/
                    this.TypesOfEnterprisesOptions=res.data.storeTypes;/*企业类型*/
                    this.companyEmployeeCountOptions=res.data.scaleList;/*员工总数*/
                }
            })
        }
    }
