<template>
	<div class="procurementOne" v-wechat-title="$route.meta.title">
...
<script>
    ...
    request(){
        ...
        this.$vux.loading.show({
          text:'Loading'
    		...
    		this.$api.post('http://116.196.89.209:9004/open/api/chance/buyChancesInfo', this.pramas, res => {
    			if (res.status >= 200 && res.status < 300) {
  ...
  mounted () {
      this.$store.commit('PROCUREMENT', '商机')
      this.pramas.keyWord = this.keyword
    	this.requestData()
      this.scrollFn()
