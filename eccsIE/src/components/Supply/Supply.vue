            ...
            <div class="banner">
                <div class="left">
                    ...
                    <ul class="proClassify">
                        <li @click="proClassifyClick(idxClassification)" :key="idxClassification" v-for="(classification,idxClassification) in products">
...
export default {
  ...
  methods: {
    proClassifyClick(index){
      $(".proClassify>li").find(".classification").css({ display: "none" });
      $(".proClassify>li").eq(index).find(".classification").css({ display: "block" });
    },
    // 搜索
    handleSearch () {
        if(this.sertchValue){
  ...
  mounted () {
    ...
    this.server(url, api.portSupply)
      .then(res => {
    ...
    this.server(supplyInfoData, api.portSupply)
      .then(res => {
        this.supplys = res.data.goodsTypeList;
      })
      .then(() => {
