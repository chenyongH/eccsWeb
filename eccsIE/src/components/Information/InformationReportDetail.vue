                ...
                <el-carousel-item v-for="(banner,idx) in banners" :key="idx">
                  <div v-if="banner.cmsBanner.type===1">
                      <div class="banner_imgBox" :style="{ float: 'left !important',	width: '270px !important', margin: '42px 0px 45px 50px !important'}">
                          <img :style="{ display:'inline-block !important',position: 'relative !important',height: '200px !important', width: '270px !important',cursor: 'pointer !important'}"  v-bind:src='imgUrl+banner.cmsBanner.picurl' alt="" @click="openNews(banner.cmsArticle.id,banner.cmsBanner.url,banner.cmsArticle.channelId)">
                      </div>
                      <div class="banner_ifo" :style="{ float:'left !important','text-align': 'left !important',color: '#ffffff !important', margin: '70px 0px 0px 30px !important','word-wrap': 'break-word !important','word-break': 'normal !important',width: '738px !important'}">
                          <p class="banner_tittle" :style="{'font-size': '32px !important',cursor: 'pointer !important'}" @click="openNews(banner.cmsArticle.id,banner.cmsBanner.url,banner.cmsArticle.channelId)">{{ banner.cmsBanner.title }}</p>
                  ...
                  <div v-else-if="banner.cmsBanner.type===2">
                      <div class="banner_imgBox" :style="{width:'100% !important',height:'286px !important'}">
                          <img :style="{width:'100% !important',height:'100% !important'}"  v-bind:src="imgUrl+banner.cmsBanner.picurl" alt="" @click="openNews(banner.cmsArticle.id,banner.cmsBanner.url,banner.cmsArticle.channelId)">
                      </div>
                   </div>
                  <div  v-else>                
                    <div class="banner_ifo" :style="{ float:'left !important','text-align': 'left !important',color: '#ffffff !important', margin: '70px 0px 0px 50px !important','word-wrap': 'break-word !important','word-break': 'normal !important',width: '1100px !important','padding-right':'80px !important'}">
                        <p class="banner_tittle" :style="{'font-size': '32px !important',cursor: 'pointer !important'}" @click="openNews(banner.cmsArticle.id,banner.cmsBanner.url,banner.cmsArticle.channelId)">{{ banner.cmsBanner.title }}</p>
                        <p class="banner_text" :style="{'font-size': '14px !important','margin-top':'20px !important','line-height': '20px !important'}">{{ banner.cmsBanner.info}}</p>
                    </div>
                  </div>
        ...
        <div class="details_content">
            <div class="content_left">
                <div class="details_news">
                    ...
                    <div class="nothing" v-if="details.length<=0 ">暂无数据</div>                    
                    <el-row >
                        <el-col v-if="detail"  v-for="(detail,idx) in details" :key="idx"  backgroundcolor="#f8f8f9" >
                            <el-card  class="details_news_content">
                                ...
                                <div  class="details_news_right">
                                    <router-link :to="{path:'/ReportIntl',query:{articleId:detail.article.id,channelId:detail.article.channelId}}">
...
export default {
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      if (flag === "more") {
        api
          .$http(api.servers + "/open/api/cms/moreNews", pa)
          .then(res => {
        ...
        api
          .$http(api.servers + "/open/api/cms/getNewsByTag", tagp)
          .then(res => {
            this.details = res.data.mapList;
            this.pageCount = res.data.totalPage;
            this.channelname = res.data.channelName;
    ...
    getData() {
      ...
      if (flag == "more") {
        api
          .$http(api.servers + "/open/api/cms/moreNews", pa) //更多新闻
          .then(res => {
            this.details = res.data.mapList;
            this.pageCount = res.data.totalPage;
            this.channelname = res.data.channelName;
